...
class TestCommandFlow:
    ...
    def test_add_then_search_flow(self, temp_db_path):
        """Test a complete flow of adding a command then searching for it."""
        # Setup mock embeddings
        mock_embedding1 = [0.1] * 1536
        mock_embedding2 = [0.11] * 1536  # Slightly different to ensure it's a close match
        ...
        search_args = Namespace(
            description="Show files with colors"
        )
        ...
        with patch('src.commands.calculate_embedding', return_value=mock_embedding2):
            with patch('src.commands.fastcmd_print') as mock_print:
