...
class TestAddCommand:
    ...
    def test_add_command_success(self, temp_db_path):
        # Arrange
    ...
    def test_add_command_handles_exception(self, temp_db_path):
        # Arrange
        args = Namespace(description="List all files", commandrun="ls -la")
...
class TestSearchCommand:
    ...
    def test_search_command_no_results(self, temp_db_path):
        # Arrange
    ...
    def test_search_command_found_but_canceled(self, temp_db_path):
        # Arrange
        args = Namespace(description="Find existing command")
        mock_embedding = [0.1] * 1536
        mock_results = [
            {"command": "ls -la", "description": "List all files", "distance": 0.1}
        ]
    ...
    def test_search_command_execute_success(self, temp_db_path):
        # Arrange
        args = Namespace(description="Find existing command")
        mock_embedding = [0.1] * 1536
        mock_results = [
            {"command": "echo hello", "description": "Print hello", "distance": 0.1}
        ]
        ...
        # Act
        with patch('src.commands.calculate_embedding', return_value=mock_embedding):
            with patch('src.commands.fetch_similar', return_value=mock_results):
    ...
    def test_search_command_execution_failure(self, temp_db_path):
        # Arrange
        args = Namespace(description="Find existing command")
        mock_embedding = [0.1] * 1536
        mock_results = [
            {"command": "invalid_command", "description": "Invalid command", "distance": 0.1}
        ]
