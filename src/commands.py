...
def handle_search(args, context):
    ...
    try:
        ...
        for i, result in enumerate(results):
            ...
            fastcmd_print(f"{i+1}. [{distance_percent}% match] {result['description']}")
            fastcmd_print(f"   Command: {result['command']}")
        
        # Ask user if they want to execute a command
        fastcmd_print("Enter the number of the command to execute, or press Enter to cancel:")
        choice = get_user_input()
        
        if not choice or not choice.isdigit() or int(choice) < 1 or int(choice) > len(results):
